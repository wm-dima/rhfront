    ...
    <template v-if="type === 'image-regular'  || type === 'image-circle' ">
      <div class="file-input" :class="type">
        ...
        <div class="button-container">
          <md-button class="md-danger md-round" @click="removeImage" v-if="hasImage"><i class="fa fa-times"></i>Remove</md-button>
    ...
    <template v-else-if="type === 'input'">
      <template v-if="!withButton">
        <md-field class="form-file-upload form-file-simple">
          <md-input v-model="inputModel" :id="inputName + 'Visible'" @click="triggerInputFileHidden(inputName + 'Hidden')" :placeholder="placeholder"></md-input>
          <input type="file" :id="inputName + 'Hidden'" @change="newValue(inputName + 'Visible', inputName + 'Hidden')">
        </md-field>
      </template>
      <template v-if="withButton">
        <div class="form-file-group">
          <md-field class="form-file-upload form-file-simple">
            <md-input v-model="inputModel" :id="inputName + 'Visible'" :multiple="multiple" @click="triggerInputFileHidden(inputName + 'Hidden')" :placeholder="placeholder"></md-input>
            <input type="file" :id="inputName + 'Hidden'" @change="newValue(inputName + 'Visible' , inputName + 'Hidden')" v-if="!multiple">
            <input type="file" :id="inputName + 'Hidden'" :multiple="multiple" @change="newValueMultiple(inputName + 'Visible' , inputName + 'Hidden')" v-else>
          </md-field>
          <md-button class="md-success md-round md-just-icon" @click="triggerInputFileHidden(inputName + 'Hidden')"><md-icon>{{ btnIcon }}</md-icon></md-button>
        </div>
      </template>
    </template>
  </span>
